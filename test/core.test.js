...
test("classifies todos by obvious domain", () => {
  assert.equal(classifyTodo("Tune prompt for agent workflow"), "AI");
...
test("renders grouped todos without color", () => {
  const output = renderTodos(
    [
      { done: false, label: "AI", text: "Tune prompt", created: "2026-05-08T00:00:00.000Z" },
  ...
  assert.match(output, /Tune prompt/);
